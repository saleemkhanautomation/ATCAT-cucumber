...
Scenario: Create Recon type
   Given Open browser in incognito and navigate to "https://reconadvisor3-dev.accenture.com/"
    ...
    And I wait for "10000" seconds
    Then I type at "Name1path" value "DEVTypeGLSanyy6"
    ...
    And I wait for "10000" seconds
   # Then select the value "Business Unit-Department-Account-Operating Unit" from the dropdown "ReconIDSIgnature1"
     And I wait for "10000" seconds
    Then Navigate to "PlusIcon"
    And I wait for "10000" seconds
    Then select the value "TestingGLSany2" from the dropdown "DataSet1Dropdown"
    And I wait for "10000" seconds
    Then select the value "TestingBankSan1" from the dropdown "DataSet2Dropdown"
    # Here in the above 2 lines type the datasets name that needs to selected from the dropdown
    And I wait for "10000" seconds
 #   Then select the value "Business Unit-Department-Account-Operating Unit" from the dropdown "ReconIDSigns"
    Then Navigate to "ReconGroupSaveButton"
    And I wait for "10000" seconds
    
    Then I verify whether the value "DEVTypeGLSanyy6" is present in the "ReconGroupTypeLocation"
   ...
   Scenario: Configure the workflow of the Recon Group type
    ...
    Then I will select " Matching" from Action dropdown
		Then I will select " TestingGLSany2" from LeftDataSet dropdown
 		#Then I will select "example" from LeftRecords dropdown
 		Then I will select " TestingBankSan1" from RightDataSet dropdown
   ...
   Scenario: Create Rule
    ...
    Then pass your "ReconGroupTypeWhenDev" in "RuleWhen"
