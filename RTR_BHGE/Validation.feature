Feature: Creation of Validation
  ...
  Scenario: Add new validation settings
    ...
    Then I type at "ValidationGroupNamePath" value "Msk1234"
  ...
  Scenario: Delete Validation Group
    Given I pass the validation name "Test123" to be deleted from the validation table
  ...
  Scenario: Configure Checklist of Validation Group
    Given I pass the validation name "Msk1234" whose Configure checklist has to be selected from the validation table
  ...
  Scenario: Configure Datasets of Validation Group
    Given I pass the validation name "Msk1234" whose Configure datasets has to be selected from the validation table
  ...
  Scenario: Update Validation Group
    Given I pass the validation name "Msk1234" that has to be updated from the validation table
  ...
  Scenario: View BackUp button
    ...
    And I wait for "3000" seconds
    Then I enter value "C9712-7777-196650-0000" in "SearchBox" field
  ...
  #  Then Navigate to "InProgress"
    ...
    Then select the value " ICAR" from the dropdown "ICARDrop"
    And I wait for "4000" seconds
  #  Then select the value " L" from the dropdown "ICARDrop"
    Then select the value " BHGE" from the dropdown "ResponsibleDrop"
    And I wait for "4000" seconds
    Then select the value " Yes" from the dropdown "DragandDrop"
    And I wait for "4000" seconds
